...
public interface ChallengeRepository extends CrudRepository<ChallengeEntity, Long> {
  ...
  @Query(value = "select new com.capgemini.coedevon.codewar.challenge.to.ChallengeTo(id, user.id, status.id, status.name, status.code, name, creationDate, startDate, endDate, multipleTries, (select count(1) from ParticipationEntity p where challenge.id = c.id)) "
